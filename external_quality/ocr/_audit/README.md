...
* `.tmp/external_quality_ocr_preview/`
...
* `relative_path` must be repo-relative under `external_quality/ocr/` when
  present
...
```bash
shasum -a 256 markitdown-quality-lab/external_quality/ocr/self_synthetic/samples/layout/sample.png
```
...
Current helpers still target `markitdown-quality-lab/external_quality/ocr/_legacy_samples/`, which
...
* these helpers validate and compare the existing `_legacy_samples` scaffold
* they are optional audit helpers, not a public-only gate
* `external_quality/ocr/_audit` is a planning/audit surface and does not
  replace current helper inputs today

## Seed Preview Smoke

For tracked OCR sample files referenced from `manifest.tsv`:

* use local `tesseract` smoke runs only as qualitative preview, not as product
  assertions
* local helper:
  `bash markitdown-quality-lab/external_quality/ocr/_audit/tools/preview_smoke.sh`
* prerequisites: `tesseract` executable plus any needed language data such as
  `eng`, `ara`, or `chi_sim`
* default output dir is `.tmp/external_quality_ocr_preview/`
* helper also writes `.tmp/external_quality_ocr_preview/preview_summary.tsv`
