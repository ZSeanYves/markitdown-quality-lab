...
def discover_corpus_root(
    ...
    return first_existing_dir(
        [
            ...
            quality_lab_root / "corpus" if quality_lab_root else None,
            repo_root / "markitdown-quality-lab" / "corpus",
            repo_root.parent / "markitdown-quality-lab" / "corpus",
...
def discover_layout_lab_root(
    ...
    if quality_lab_root is not None:
        return first_existing_dir([quality_lab_root / "pdf_layout_classifier"])
    return first_existing_dir(
        [
            repo_root / "markitdown-quality-lab" / "pdf_layout_classifier",
            repo_root.parent / "markitdown-quality-lab" / "pdf_layout_classifier",
        ]
    )
...
def discover_model_root(
    ...
    return first_existing_dir(
        [
            ...
            (quality_lab_root / "pdf_layout_classifier" / "models") if quality_lab_root else None,
            (layout_lab_root / "models") if layout_lab_root else None,
            repo_root / "markitdown-quality-lab" / "pdf_layout_classifier" / "models",
            repo_root.parent / "markitdown-quality-lab" / "pdf_layout_classifier" / "models",
...
def default_feature_dir(layout_lab_root: Path | None, repo_root: Path = REPO_ROOT) -> Path:
    if LAYOUT_LAB_ROOT.is_dir():
        return LAYOUT_LAB_ROOT / "local_eval" / "features" / "raw"
    if layout_lab_root:
        return layout_lab_root / "local_eval" / "features" / "raw"
    return repo_root / ".tmp" / "pdf_layout_classifier" / "features"


def default_eval_output_dir(layout_lab_root: Path | None, repo_root: Path = REPO_ROOT) -> Path:
    if LAYOUT_LAB_ROOT.is_dir():
        return LAYOUT_LAB_ROOT / "local_eval" / "eval"
    if layout_lab_root:
        return layout_lab_root / "local_eval" / "eval"
...
def default_model_output_path(
    ...
    if layout_lab_root:
        return layout_lab_root / "models" / name
    return repo_root / ".tmp" / "pdf_layout_classifier" / "models" / name
